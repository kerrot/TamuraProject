...
public static class Uni2DEditorSpriteBuilderUtils 
{
	...
	public static GameObject GenerateSpriteFromSettings( Uni2DEditorSpriteSettings a_rNewSpriteSettings, Uni2DSprite a_rSprite = null, bool a_bForceRebuild = false, bool a_bInABatch = false )
	{
		...
		bool bRegisterUpdateUndo = ms_bUndoEnabled && bUpdating && !a_bForceRebuild;
		...
		if( a_bInABatch == false )
		{
			...
			if( rTextureImporter != null )
			{
				if( bRegisterUpdateUndo )
				{
					...
					if(bUpdating)
					{
						Uni2DUndo.RegisterFullObjectHierarchyUndo(a_rSprite.gameObject, "Update Uni2D sprite");
					}
					else
					{
	...
	public static GameObject GenerateSpriteMeshFromSettings( Uni2DEditorSpriteSettings a_rNewSpriteSettings, Uni2DSprite a_rSprite )
	{
		...
		rCurrentSpriteData.scaleMode          = a_rNewSpriteSettings.spriteScaleMode;
	...
	public static GameObject GenerateSpriteMatFromSettings( Uni2DEditorSpriteSettings a_rNewSpriteSettings, Uni2DSprite a_rSprite )
	{
		...
		{
			if( a_rSprite.Bones.Count > 0 && a_rSprite.NeedSkinMeshComponent)
			{
	...
	{
		a_rSprite.RestoreSkeletonPosePosition( );
	...
	{	
		...
		else
		{
			...
			Vector2 f2RealScale           = rSpriteSettings.ScaleFactor;
			...
			// The delta to apply
			Vector2 f2ScalingDelta        = f2RealScale;
			f2ScalingDelta.x /= rSpriteData.Scale.x;
			f2ScalingDelta.y /= rSpriteData.Scale.y;
			
			Vector3 f3ScaledDeltaPivot = ( f2NewPivotCoords - f2CurrentPivotPoint );
			f3ScaledDeltaPivot.x *= f2RealScale.x;
			f3ScaledDeltaPivot.y *= f2RealScale.y;
			...
			for( int iMeshIndex = 0, iMeshCount = rSpriteData.meshCollidersList.Count; iMeshIndex < iMeshCount; ++iMeshIndex )
			{
				...
				rMeshCollider.isTrigger = rSpriteSettings.isTrigger;
			}
			
			#if AFTER_UNITY_4_3
			Uni2DMesh2D rMesh2D = rSpriteData.mesh2D;
			if(rMesh2D != null)
			{
				foreach(Uni2DMeshShape2D rShape in rSpriteData.mesh2D.shapes)
				{
					Vector2[] oVertices = rShape.vertices;
					for(int i = 0; i < oVertices.Length; ++i)
					{
						Vector2 f2Vertex = oVertices[i];
						f2Vertex -= (Vector2)f3ScaledDeltaPivot;
						f2Vertex.x *= f2ScalingDelta.x;
						f2Vertex.y *= f2ScalingDelta.y;
						oVertices[i] = f2Vertex;
					}
				}
			}
			
			PolygonCollider2D rPolygonCollider2D = rSpriteData.polygonCollider2D;
			if(rPolygonCollider2D != null)
			{
				if(rMesh2D != null)
				{
					rMesh2D.CopyVertices(rPolygonCollider2D);
				}
				rPolygonCollider2D.isTrigger = rSpriteSettings.isTrigger;	
			}
			
			/*PolygonCollider2D rPolygonCollider2D = rSpriteData.polygonCollider2D;
			if(rPolygonCollider2D != null)
			{
				for(int iShapeIndex = 0; iShapeIndex < rPolygonCollider2D.pathCount; ++iShapeIndex)
				{
					Vector2[] oPoints = rPolygonCollider2D.GetPath(iShapeIndex);
					for(int i = 0; i < oPoints.Length; ++i)
					{
						Vector2 f2Vertex = oPoints[i];
						f2Vertex -= (Vector2)f3ScaledDeltaPivot;
						f2Vertex.x *= f2ScalingDelta.x;
						f2Vertex.y *= f2ScalingDelta.y;
						oPoints[i] = f2Vertex;
					}
					rPolygonCollider2D.SetPath(iShapeIndex, oPoints);
				}
				rPolygonCollider2D.isTrigger = rSpriteSettings.isTrigger;
			}*/
			#endif
			...
			// Apply delta to skeleton
			Vector3 f3OffsetManipulatedPosition = -f3ScaledDeltaPivot;
			f3OffsetManipulatedPosition.x *= f2ScalingDelta.x;
			f3OffsetManipulatedPosition.y *= f2ScalingDelta.y;
			a_rSprite.OffsetSkeletonManipulatedPosition(f3OffsetManipulatedPosition);
			//a_rSprite.RestoreSkeletonPosePosition( );
			...
			rSpriteData.scaleMode = rSpriteSettings.spriteScaleMode;
	...
	public static void OnPrefabPostProcess(GameObject a_rPrefab)
	{
		...
		{	
			...
			if(bNeedToBeSaved)
			{
				...
				ReplaceSpritePrefab(rPrefabInstance, a_rPrefab);
	...
	public static void UpdateSpriteInResource(Uni2DSprite a_rSprite, bool a_bForce = false )
	{
		...
		// Instantiate the prefab
		GameObject rPrefabInstance = InstantiateSpritePrefab(rPrefab);
